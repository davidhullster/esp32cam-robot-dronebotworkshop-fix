...
void setup() 
{
  ...
  // Setup LED PWM using standard ESP-IDF LEDC functions
  ledc_timer_config_t ledc_timer_led;
  ledc_timer_led.speed_mode = LEDC_LOW_SPEED_MODE;
  ledc_timer_led.timer_num = LEDC_TIMER_1;
  ledc_timer_led.freq_hz = 5000;
  ledc_timer_led.duty_resolution = LEDC_TIMER_8_BIT;
  ledc_timer_led.clk_cfg = LEDC_AUTO_CLK;
  ledc_timer_config(&ledc_timer_led);

  ledc_channel_config_t ledc_channel_led;
  ledc_channel_led.speed_mode = LEDC_LOW_SPEED_MODE;
  ledc_channel_led.channel = LEDC_CHANNEL_1;
  ledc_channel_led.timer_sel = LEDC_TIMER_1;
  ledc_channel_led.gpio_num = 4;  // pin4 is LED
  ledc_channel_led.duty = 0;
  ledc_channel_led.hpoint = 0;
  ledc_channel_led.intr_type = LEDC_INTR_DISABLE;
  ...
  for (int i=0;i<5;i++) 
  {
    ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, 10);  // flash led
